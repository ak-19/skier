...
import { ObstacleManager } from "../Sprites/Obstacles/ObstacleManager";
import { Rhino } from "../Sprites/Rhino";
...
export class Game {
  ...
  private rhinoRuningState: Boolean = true;
  ...
  rhinoStateChenged(runing: Boolean) {
    this.rhinoRuningState = runing;
  }
  ...
  drawGameWindow() {
    ...
    StatsText.drawGameStats(this.canvas, this.skier);
    if (!this.rhinoRuningState) StatsText.drawGameOver(this.canvas);
  }
  ...
  startNewGame(){
    ...
    this.rhino = new Rhino(-500, -2000, this.imageManager, this.canvas, this.rhinoStateChenged.bind(this));
  
    this.calculateGameWindow();
    this.obstacleManager.placeInitialObstacles();
    this.rhinoRuningState = true; 
  }
  ...
  handleKeyDown(event: KeyboardEvent) {
    if (!this.rhinoRuningState && event.key === 'Enter') this.startNewGame();
