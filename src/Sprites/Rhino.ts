...
export class Rhino extends Sprite {
  /**
  ...
  curAnimationFrameTime: number = Date.now();
    ...
    /**
   * When rhino changes state active or running, this callback is fired.
   */
  notifyGameOver: Function | undefined = undefined;
  ...
  constructor(
    ...
    this.setAnimation();
    this.notifyGameOver = notifyGameOver;
  }
   ...
   */
  update(gameTime: number, target: Sprite) {
    if (this.isRunning()) {
      this.move(target);
   ...
   */
  checkIfCaughtTarget(target: Sprite) {
   ...
   */
  caughtTarget(target: Sprite) {
    target.die();
    this.setState(STATES.STATE_EATING);
    this.notifyGameOver && this.notifyGameOver(false);
  }
